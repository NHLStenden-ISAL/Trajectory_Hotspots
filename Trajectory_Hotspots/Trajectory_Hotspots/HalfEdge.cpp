...
std::vector<HalfEdge*> split_edges(HalfEdge* edge1, HalfEdge* edge2, Vertex* intersection)
{
	...
	//Get the first clockwise half edge pointing away from the intersection point and set the new edge as its previous
	...
	for (size_t i = 0; i < halfedges.size(); i++)
	{
		...
		if (halfedges[i]->origin != intersection)
		{
...
HalfEdge* insert_edge(HalfEdge* halfedge)
{
